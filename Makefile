...
# agent-install.openshift.io/cluster-api-provider-agent-bundle:$VERSION and agent-install.openshift.io/cluster-api-provider-agent-catalog:$VERSION.
IMAGE_TAG_BASE ?= agent-install.openshift.io/cluster-api-provider-agent
...
test: manifests generate fmt vet envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) -p path)" go test ./... -coverprofile cover.out -ginkgo.focus="$(FOCUS)" -ginkgo.v -ginkgo.skip="$(SKIP)"
