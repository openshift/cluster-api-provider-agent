...
func newDataSecret(name, namespace, ignEndpoint string, ctx context.Context, c client.Client) *corev1.Secret {
...
func newAgentMachine(name, namespace string, spec capiproviderv1.AgentMachineSpec, ctx context.Context, c client.Client) (*capiproviderv1.AgentMachine, *clusterv1.Machine) {
	...
	secret := newDataSecret(name, namespace, "https://endpoint/ignition", ctx, c)
...
func getMachine(ctx context.Context, c client.Client, agentMachine *capiproviderv1.AgentMachine) (*clusterv1.Machine, error) {
	for _, owner := range agentMachine.ObjectMeta.OwnerReferences {
		if owner.Kind == "Machine" && (owner.APIVersion == clusterv1.GroupVersion.String() || owner.APIVersion == "cluster.x-k8s.io/v1beta1") {
			machine := &clusterv1.Machine{}
...
var _ = Describe("agentmachine reconcile", func() {
	...
	It("sets the finalizer and the machine delete hook annotation with legacy v1beta1 APIVersion", func() {
		agentMachine, _ := newAgentMachine("agentMachine-1", testNamespace, capiproviderv1.AgentMachineSpec{}, ctx, c)
		machineOwnerRef := metav1.OwnerReference{APIVersion: "cluster.x-k8s.io/v1beta1", Kind: "Machine", Name: fmt.Sprintf("machine-%s", agentMachine.Name)}
		agentMachine.ObjectMeta.OwnerReferences = []metav1.OwnerReference{machineOwnerRef}
		Expect(c.Create(ctx, agentMachine)).To(Succeed())
	...
	It("agentMachine no agents", func() {
		...
		Expect(conditions.Get(agentMachine, string(capiproviderv1.AgentReservedCondition)).Status).To(BeEquivalentTo("False"))
		Expect(conditions.Get(agentMachine, string(capiproviderv1.AgentReservedCondition)).Reason).To(Equal(capiproviderv1.NoSuitableAgentsReason))
		Expect(conditions.Get(agentMachine, string(capiproviderv1.AgentSpecSyncedCondition)).Status).To(BeEquivalentTo("False"))
		Expect(conditions.Get(agentMachine, string(capiproviderv1.AgentValidatedCondition)).Status).To(BeEquivalentTo("False"))
		Expect(conditions.Get(agentMachine, string(capiproviderv1.AgentRequirementsMetCondition)).Status).To(BeEquivalentTo("False"))
		Expect(conditions.Get(agentMachine, string(capiproviderv1.InstalledCondition)).Status).To(BeEquivalentTo("False"))
		// Note: v1beta2 conditions don't have Severity field
		Expect(conditions.Get(agentMachine, string(clusterv1.ReadyCondition)).Status).To(BeEquivalentTo("False"))
	})
	...
	It("agentMachine find agent end-to-end", func() {
		...
		Expect(conditions.Get(agentMachine, string(capiproviderv1.AgentValidatedCondition)).Status).To(BeEquivalentTo("True"))
		Expect(conditions.Get(agentMachine, string(capiproviderv1.AgentRequirementsMetCondition)).Status).To(BeEquivalentTo("True"))
		Expect(conditions.Get(agentMachine, string(capiproviderv1.InstalledCondition)).Status).To(BeEquivalentTo("False"))
		// Note: v1beta2 conditions don't have Severity field
		Expect(conditions.Get(agentMachine, string(clusterv1.ReadyCondition)).Status).To(BeEquivalentTo("False"))
	...
	It("agent bind-unbind cleanup", func() {
		...
		conditions.Set(machine, metav1.Condition{
			Type:    string(clusterv1.PreTerminateDeleteHookSucceededV1Beta1Condition),
			Status:  metav1.ConditionFalse,
			Reason:  string(clusterv1.WaitingExternalHookV1Beta1Reason),
			Message: "",
		})
	...
	It("agentMachine agent with missing agentref", func() {
		...
		Expect(amr.updateFoundAgent(ctx, log, agentMachine, agent, clusterDepRef, &BootstrapData{ignitionTokenSecretRef: &aiv1beta1.IgnitionEndpointTokenReference{Namespace: testNamespace, Name: "agent-1"}})).To(BeNil())
	...
	Context("reconciling ignition token secret", func() {
		...
		Context("ignition token secret reference changes", func() {
			It("updates the ignition token secret reference on the Agent when it changes in the Machine", func() {
				agent.ObjectMeta.Annotations = map[string]string{AgentMachineRefNamespace: testNamespace}
				Expect(c.Update(ctx, agent)).To(Succeed())
				am, m := newAgentMachine("agentMachine-2", testNamespace, capiproviderv1.AgentMachineSpec{}, ctx, c)
				m.Annotations[machineDeleteHookName] = ""
				Expect(c.Update(ctx, m)).To(Succeed())
				am.Status.AgentRef = &capiproviderv1.AgentReference{Namespace: agent.Namespace, Name: agent.Name}
				Expect(c.Create(ctx, am)).To(Succeed())
				newSecret := newDataSecret("new-secret-name", testNamespace, "https://endpoint/new-ignition", ctx, c)

				result, err := amr.Reconcile(ctx, newAgentMachineRequest(am))
				Expect(err).To(BeNil())
				Expect(result).To(Equal(ctrl.Result{}))

				By("Checking that the ignition token secret reference is set on the Agent")
				ignitionTokenSecretName := m.Spec.Bootstrap.DataSecretName
				agentCR := &aiv1beta1.Agent{}
				Expect(c.Get(ctx, types.NamespacedName{Name: agent.Name, Namespace: agent.Namespace}, agentCR)).To(Succeed())
				Expect(agentCR.Spec.IgnitionEndpointTokenReference).NotTo(BeNil())
				Expect(agentCR.Spec.IgnitionEndpointTokenReference.Namespace).To(Equal(testNamespace))
				Expect(agentCR.Spec.IgnitionEndpointTokenReference.Name).To(Equal(fmt.Sprintf("agent-%s", *ignitionTokenSecretName)))

				By("Updating the ignition token secret reference on the Machine")
				m.Spec.Bootstrap.DataSecretName = &newSecret.Name
				Expect(c.Update(ctx, m)).To(Succeed())

				By("Reconciling the AgentMachine")
				result, err = amr.Reconcile(ctx, newAgentMachineRequest(am))
				Expect(err).To(BeNil())
				Expect(result).To(Equal(ctrl.Result{}))

				By("Checking that the ignition token secret reference changed in the Agent")
				Expect(c.Get(ctx, types.NamespacedName{Name: agent.Name, Namespace: agent.Namespace}, agentCR)).To(Succeed())
				Expect(agentCR.Spec.IgnitionEndpointTokenReference).NotTo(BeNil())
				Expect(agentCR.Spec.IgnitionEndpointTokenReference.Namespace).To(Equal(testNamespace))
				Expect(agentCR.Spec.IgnitionEndpointTokenReference.Name).To(Equal(fmt.Sprintf("agent-%s", newSecret.Name)))
			})
		})
	})
})
